...
- Add production-only server secrets in Vercel: `GEMINI_API_KEY` (or, later, `OPENAI_API_KEY`), `SUPABASE_SERVICE_ROLE_KEY`, and `AERODATABOX_RAPIDAPI_KEY`. Never use `NEXT_PUBLIC_` for these values.
- Set `MOMO_AI_PROVIDER=gemini` in Vercel for the current no-cost hosted setup. Never point a Vercel deployment at a local Ollama URL.
