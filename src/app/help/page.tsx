...
export default function HelpPage() {
  return <main className="help-page"><header className="topbar"><Link className="brand" href="/"><span role="img" aria-label="Momo the panda">🐼</span><span>Momo</span></Link><Link className="nav-button" href="/">Back to Momo</Link></header><section className="help-hero"><span className="help-panda" role="img" aria-label="Friendly panda">🐼</span><p className="eyebrow">WHAT MOMO CAN HELP WITH</p><h1>You do not need to know the flight rules first.</h1><p>Choose the situation closest to yours. Momo will ask simple questions and explain why they matter.</p><Link className="help-cta" href="/">Tell Momo what happened →</Link></section><section className="help-grid">{examples.map(([title, text], index) => <article className={`help-card card-${index + 1}`} key={title}><span>✦</span><h2>{title}</h2><p>{text}</p><Link href="/">Ask Momo about this →</Link></article>)}</section><p className="help-note">Momo gives general information and communication support. It does not guarantee an outcome.</p></main>;
}
